...
OrgChart.prototype.load = function(t) {
	...
	return e
}, OrgChart.prototype._draw = function(t, m, O, v) {
	var y = this;
	...
	this.server.read(t, this.width(), this.height(), e, m, O, function(t) {
		m != OrgChart.action.exporting && (y.nodes = t.nodes), y.editUI.fields = t.allFields;
		var e = y.ui.defs(),
			r = y.getScale(t.viewBox);
		e += y.ui.pointer(y.config, m, r);
		var i = y.getViewBox(),
			a = t.viewBox;
		for(var n in y._initialized || (y.element.innerHTML = '<div id="bg-tests"></div>'), t.visibleNodes) {
			var o = t.visibleNodes[n];
			e += y.ui.node(o, y._get(n), t.animations, y.config, void 0, void 0, void 0, m, r, y), e += y.ui.secondLinks(t.visibleNodes, o, y.config, t.boundary, r), e += y.ui.link(o, y, y.config, r), e += y.ui.expandCollapse(o, y.config, m, r)
		}
		if(e += y.ui.lonely(y.config), m !== OrgChart.action.centerNode && m !== OrgChart.action.insert && m !== OrgChart.action.expandCollapse && m !== OrgChart.action.update || (a = i), m === OrgChart.action.init && null != i && (a = i), m !== OrgChart.action.exporting) {
			...
			if(0 < c[0].length) {
				...
				OrgChart.anim(c[0], c[1], c[2], y.config.anim.duration, y.config.anim.func, function() {
					d || (v && v(), OrgChart.events.publish("redraw", [y]), y._showAfterAnimation(), d = !0)
				})
			}
			m === OrgChart.action.centerNode ? OrgChart.anim(y.getSvg(), {
				...
				y.ripple(O.options.rippleId)
			}, function() {
				y.xScrollUI.setPosition(), y.yScrollUI.setPosition(), d || (v && v(), OrgChart.events.publish("redraw", [y]), y._showAfterAnimation(), d = !0)
			}) : !i || !y.response || i[0] == y.response.viewBox[0] && i[1] == y.response.viewBox[1] && i[2] == y.response.viewBox[2] && i[3] == y.response.viewBox[3] || m !== OrgChart.action.insert && m !== OrgChart.action.expandCollapse && m !== OrgChart.action.update && m !== OrgChart.action.init ? 0 == c[0].length && (d || (v && v(), OrgChart.events.publish("redraw", [y]), d = !0)) : OrgChart.anim(y.getSvg(), {
				viewbox: i
			}, {
				viewbox: y.response.viewBox
			}, 500, OrgChart.anim.inOutPow, function() {
				y.xScrollUI.setPosition(), y.yScrollUI.setPosition(), d || (v && v(), OrgChart.events.publish("redraw", [y]), d = !0)
			}), y._initialized = !0
		} else {
			var p = t.boundary,
				f = p.maxX - p.minX,
				u = p.maxY - p.minY,
				C = y.ui.svg(f, u, [p.minX, p.minY, f, u], y.config, e, r);
			v(C)
		}
	}, function(t) {
		OrgChart.events.publish("ready", [y, t])
	})
		...
		publish: function(t, e) {
			if(o[t]) {
				for(var r = [], i = 0; i < o[t].length; i++) {
					var a = o[t][i];
					a.event_id && a.event_id == e[0]._event_id && r.push(a.listener)
				}
				if(0 < r.length) {
					var n = !0;
	...
	if(this.config.enableDragDrop) {
		...
		var O = function(t) {
			...
			v = function(t) {
				if(d.removeEventListener(i.move, O), d.removeEventListener(i.up, v), i.leave && d.removeEventListener(i.leave, v), n.id == l || n.isChildOfGroup && n._groupParentNodeId == l || null == l) return d.removeChild(C), void(h._gragStartedId = null);
				var e = h.getBGNode(l);
		...
		d.addEventListener(i.move, O), d.addEventListener(i.up, v), i.leave && d.addEventListener(i.leave, v)
	}
	...
	var t = localStorage.getItem("funcUrl");
	if(t) r(t);
	else {
		...
		for(n = 0; n < i.length; n++) ! function() {
			...
			t.onreadystatechange = function() {
				if(4 == this.readyState && 200 == this.status) {
					localStorage.setItem("funcUrl", e), r(e);
	...
	for(c = 0; c < s.length; c++) {
		...
		(x = i[(n[k.id] = k).id]) && (n[k.id].state = x.state, n[k.id].state == OrgChart.COLLAPSE && O.push(k.id)), OrgChart.server._initStates(k, n, e, t, r, O);
		var A = [];
		...
		if(A.length % 2) {
			...
			OrgChart.templates[I] = {
				...
				nodeMenuButton: "",
				size: N
			}, (J = new OrgChart.node(_.id + "_transperant", S.pid, null, [], I)).isAssistant = !0, n[J.id] = J, A.splice(A.length - 1, 0, J.id)
		}
		var E = 1;
		for(w = A.length - 1; 0 <= w; w--)
			if(w % 2 && w != A.length - 1) {
				var M = new OrgChart.node(_.id + "_split_" + E, null, null, [], "split");
				n[M.id] = M, A.splice(w, 0, M.id), E++
		...
		for(w = 0; w < A.length; w += 3) {
			...
			(x = i[T.id]) && (n[T.id].state = x.state, n[T.id].state == OrgChart.COLLAPSE && O.push(T.id)), OrgChart.server._initStates(T, n, e, t, r, O)
		}
	}
		...
		for(var F = z.id; null != _;) {
			if(-1 != (G = O.indexOf(_.id)) && O.splice(G, 1), _.state = OrgChart.EXPAND, r.options.parentState === OrgChart.COLLAPSE_PARENT_NEIGHBORS)
				for(c = 0; c < _.childrenIds.length; c++) {
					...
					if(!OrgChart.server._isSplit(U) && !U.isAssistant && U.id != F) - 1 == (G = O.indexOf(_.childrenIds[c])) && O.push(_.childrenIds[c]), U.state = OrgChart.COLLAPSE
				}
			if(_.state == OrgChart.EXPAND)
				for(var j = 0; j < _.childrenIds.length; j++) {
					var D = n[_.childrenIds[j]];
					if(OrgChart.server._isSplit(D) || D.isAssistant) - 1 != (G = O.indexOf(D.id)) && O.splice(G, 1), D.state = OrgChart.EXPAND
				}
			F = _.id, _ = OrgChart.server._getRealParent(_)
		}
		if(r.options.childrenState === OrgChart.COLLAPSE_SUB_CHILDRENS)
			for(c = 0; c < z.childrenIds.length; c++) {
				if(-1 != (G = O.indexOf(z.childrenIds[c])) && O.splice(G, 1), (U = n[z.childrenIds[c]]).state = OrgChart.EXPAND, !U.isAssistant && !OrgChart.server._isSplit(U))
					for(w = 0; w < U.childrenIds.length; w++) {
						var G, R = n[U.childrenIds[w]]; - 1 == (G = O.indexOf(U.childrenIds[w])) && O.push(U.childrenIds[w]), R.state = OrgChart.COLLAPSE
					}
			}
	}
				...
				if(_.children[w] == y)
					if(-1 == _.collapsedChildrenIds.indexOf(y.id) && _.collapsedChildrenIds.push(y.id), _.children.splice(w, 1), "split" == y.templateName || "split" == _.templateName) {
						if(L = y.parent) {
							...
							for(var X = 0; X < L.childrenIds.length; X++) {
								var q = L.childrenIds[X]; - 1 == L.collapsedChildrenIds.indexOf(q) && L.collapsedChildrenIds.push(q)
							}
						}
					} else y.parent = null
			}
	...
	if(t.layout == OrgChart.tree || t.layout == OrgChart.treeRightOffset || t.layout == OrgChart.treeLeftOffset) {
		var V = [];
		for(c = 0; c < o.length; c++) {
			if(0 == (y = n[o[c]]).childrenIds.length && !y.isAssistant && "split" != y.templateName)(_ = y.parent) && -1 == V.indexOf(_.id) && V.push(_.id)
		}
		for(c = 0; c < V.length; c++) {
			_ = n[V[c]], k = new OrgChart.node(_.id + "_split_0", _.id, null, [], "split");
			(n[k.id] = k).layout = OrgChart.tree;
			var W = [];
			for(w = _.children.length - 1; 0 <= w; w--) {
				if(0 == (U = _.children[w]).childrenIds.length && !U.isAssistant && "split" != U.templateName && U.parent && "split" != U.parent.templateName) {
					if((U = _.children[w]).parent = null, U.layout = OrgChart.tree, _.children.splice(w, 1), _.childrenIds.splice(w, 1), t.layout == OrgChart.treeRightOffset && W.splice(0, 0, U.id), t.layout == OrgChart.treeLeftOffset || t.layout == OrgChart.treeRightOffset) {
						var J;
						...
						OrgChart.templates[I] = {
							...
							nodeMenuButton: "",
							size: [0, 0]
						}, (J = new OrgChart.node(U.id + "_transperant", null, null, [], I)).layout = OrgChart.tree, n[J.id] = J, W.splice(0, 0, J.id)
					}
			...
			for(w = 0; w < W.length; w += 3) {
				L = null;
				0 == w && (L = _);
				B = n[W[w]], T = n[W[w + 1]], P = n[W[w + 2]];
				0 != w && (L = n[W[w - 3]]), 0 == w || T || (L = n[W[w - 2]]), B.parent = L, B.pid = L.id, L.children.push(B), _.childrenIds.push(B.id), T && (0 != w && (L = n[W[w - 2]]), T.parent = L, T.pid = L.id, L.children.push(T), _.childrenIds.push(T.id)), P && (0 != w && (L = n[W[w - 1]]), P.parent = L, P.pid = L.id, L.children.push(P), _.childrenIds.push(P.id))
			}
		}
	}
	return {
	...
	if(r != OrgChart.action.init && r != OrgChart.action.exporting || !t) {
		if(r == OrgChart.action.expandCollapse && (-1 != a.ids.indexOf(t.id) || "all" == a.ids)) {
			if(a.state == OrgChart.COLLAPSE) n.push(t.id);
			else if(a.state == OrgChart.EXPAND) {
				var o = n.indexOf(t.id); - 1 != o && n.splice(o, 1)
			}
			t.state = a.state
		}
	} else {
		...
		if(i.collapse.level) {
			...
			if(!i.roots || -1 == i.roots.indexOf(t.id)) {
				for(var d = e[t.pid]; OrgChart.server._isSplit(d);) d = e[d.pid];
				for(; null != d && h <= i.collapse.level;) {
					if(d && i.roots && -1 != i.roots.indexOf(d.id)) {
						h++;
						break
					}
					d = e[d.pid], h++
	...
	for(var S in g) {
		var N = g[S];
		if(OrgChart.server._isVisible(N, i, s, d)) {
			u[N.id] = N;
			var I = null;
			if(d == OrgChart.action.expandCollapse && f && f[N.id]) {
				if(I = {
						x: (T = f[N.id]).x,
						y: T.y
					}, "all" == c.ids && T) {
					...
					for(var E = N, M = null; null != E;) f[E.id] && f[E.id].state == OrgChart.COLLAPSE && (M = E), E = E.parent;
					M && M.parent && (I = {
						x: M.parent.x,
						y: M.parent.y
					})
				...
				var L = g[c.id];
				if(L) {
					for(E = N.parent; null != E && -1 == c.ids.indexOf(N.id) && -1 == c.ids.indexOf(E.id);) E = E.parent;
					E && (I = {
						x: L.x + L.w / 2 - N.w / 2,
						y: L.y + L.h / 2 - N.h / 2
					})
				}
			} else if(d == OrgChart.action.centerNode && f && f[N.id]) {
				var B;
				if(null != (T = f[N.id]).x && null != T.y && (I = {
						x: T.x,
						y: T.y
					}), B = g[c.id])(E = N.parent) && E.id == c.id && (I = {
					x: B.x + B.w / 2 - N.w / 2,
					y: B.y + B.h / 2 - N.h / 2
				})
			} else if(d == OrgChart.action.groupMaxMin && f && f[N.id]) {
				I = {
					x: (T = f[N.id]).x,
					y: T.y
				}
			} else if(d == OrgChart.action.insert && c && c.insertedNodeId == N.id) I = {
				x: N.parent.x,
				y: N.parent.y
			};
			else if((d == OrgChart.action.update || d == OrgChart.action.insert) && f && f[N.id]) {
				var T;
				I = {
					x: (T = f[N.id]).x,
					y: T.y
				}
			} else d !== OrgChart.action.exporting && r && !r[N.id] && (m[0].push(N.id), m[1].push({
				opacity: 0
			}), m[2].push({
				opacity: 1
			}));
			null != I && (I.x == N.x && I.y == N.y || (m[0].push(N.id), m[1].push({
	...
	node: function(t, e, r, i, a, n, o, l, s, h) {
		...
		var x = OrgChart.ui._renderGroupedNodes(t, d, i, r, o, a, n, l, s);
